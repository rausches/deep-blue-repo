...
namespace Uxcheckmate_Main.Services
{
    public class PdfExportService
    {
        public byte[] GenerateReportPdf(Report report)
        {
            ...
            string backgroundImagePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "images", "pdf_bg.JPG");
            ...
            var document = Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Content().Column(col =>
                    {
                        ...
                        {
                            // Section Title
                            section.Item().Background(Colors.Grey.Lighten4).Padding(10).Column(innerSection =>
                            {
                                innerSection.Item().Text("Design Issues").Bold().FontSize(20);
                        ...
                        col.Item().Container().Background(Colors.Grey.Lighten3).Padding(10).Column(section =>
                        {
                            ...
                            section.Item().Background(Colors.Grey.Lighten4).Padding(10).Column(innerSection =>
                            {
                                innerSection.Item().Text("Accessibility Issues").Bold().FontSize(20);
        ...
        #if DEBUG
                    // In development, generate PDF and optionally save it for preview
                    var pdfBytes = document.GeneratePdf();            
                    return pdfBytes;
        #else
                    // In production, generate real PDF
